...
async function getNextBaptismId() {
  try {
    ...
    const numericMatch = String(maxId).match(/\d+$/);
    if (numericMatch) {
...
async function createWaterBaptism(baptismData) {
  try {
    ...
    if (!is_member && (!firstname || !lastname)) {
      throw new Error('Missing required fields: firstname and lastname are required for non-member registration');
    }

    // Check if email already exists in accounts table (for non-members)
    // If it does, we find the matching member record to link the baptism.
    if (!is_member && email && email.trim() !== '') {
      const emailCheck = await checkDuplicateEmail(email);
      if (emailCheck.isDuplicate) {
        console.log(`[CreateWaterBaptism] Email ${email} already has an account. Looking up member...`);
        const [memRows] = await query('SELECT member_id FROM tbl_members WHERE LOWER(TRIM(email)) = LOWER(TRIM(?))', [email]);
        if (memRows.length > 0) {
          member_id = memRows[0].member_id;
          is_member = true;
          console.log(`[CreateWaterBaptism] Linked to member_id: ${member_id}`);
        }
      }
    }
...
async function bulkDeleteWaterBaptisms(baptismIds, archivedBy = null, reason = null) {
  try {
    ...
    const validIds = baptismIds
      .map(id => {
        if (typeof id === 'string') return id.trim();
        if (typeof id === 'number') return String(id).padStart(10, '0');
        return null;
      })
...
async function processBaptismCompletion(baptismId, customDateCreated = null) {
  try {
    ...
    // Normalize email (handle 'null' string from JS/DB and whitespace)
    ...
    if (!email) {
      ...
      if (memberCheck.success && memberCheck.data && memberCheck.data.email) {
        email = memberCheck.data.email;
        // Normalize the email from member record too
        if (email === 'null' || email === 'undefined') email = null;
        email = email && typeof email === 'string' ? email.trim() : null;
    ...
    // If still no email, use a unique placeholder to satisfy DB constraints and allow account creation
    if (!email) {
      email = `member.${baptismId}@bbek-placeholder.com`;
      console.log(`No email found for baptism ${baptismId}. Using placeholder: ${email}`);
    }
