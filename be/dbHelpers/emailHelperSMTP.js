...
const sendAccountDetails = async (accountDetails) => {
  try {
    ...
    // Generate cryptographically secure token if not provided
    const crypto = require('crypto');
    const token = accountDetails.token || accountDetails.resetToken || crypto.randomBytes(32).toString('hex');
    ...
    const htmlContent = getEmailHeader(title) + `
...
const sendWaterBaptismDetails = async (baptismDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader('Water Baptism Service Update') + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
...
const sendBurialServiceRequestNotification = async (requestDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader('Burial Service Request Received') + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
...
/**
 * Send burial service status update email
 */
const sendBurialDetails = async (burialDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader('Burial Service Update') + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
...
const sendChildDedicationDetails = async (dedicationDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader('Child Dedication Service Update') + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
...
const sendMarriageDetails = async (marriageDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader('Marriage Service Update') + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
...
const sendFormSubmissionNotification = async (formDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader(`${formTypeLabel} Received`) + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
...
const sendFormStatusUpdate = async (formDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader(`${formTypeLabel} Update`) + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
...
const sendApprovalRequestNotification = async (approvalDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader(`${typeLabel} Join Request Received`) + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
...
const sendApprovalStatusUpdate = async (approvalDetails) => {
  try {
    ...
    const htmlContent = getEmailHeader(`${typeLabel} Request Update`) + `
      <p style="font-size: 16px; color: #555;">Dear ${recipientName},</p>
      <p style="font-size: 15px; color: #555;">${statusMessages[status] || 'Your request status has been updated.'}</p>
      ${getStatusBadge(status, {}, statusMessages)}
      ${getInfoBox('Request Details', [
