/**
 * AUDIT TRAIL MIDDLEWARE v2 - Raw Data Only
 * All data stored as plain text, no Buffers, no IDs
 */

const { createAuditLog } = require('../dbHelpers/auditTrailRecords_v2');
...
/**
 * Audit trail middleware v2
 * Logs all requests as plain text data
 */
const auditTrailMiddleware = async (req, res, next) => {
  ...
  res.on('finish', async () => {
    try {
      ...
      // Build audit data (all plain text)
      const auditData = {
