...
/**
 * GET - Get header data with content and images (combined)
 * GET /api/cms/header/full
 * Returns: { page: {...}, images: {...} }
 */
router.get('/header/full', async (req, res) => {
  try {
    console.log('[CMS] Loading header data...');
    
    // Get page content
    const pageResult = await getCmsPage('header');
    
    // Get all images for header
    const imagesResult = await getAllCmsImages('header');
    
    // Format images object
    let images = {};
    if (imagesResult.success && imagesResult.data) {
      imagesResult.data.forEach(img => {
        if (img.fieldName && img.imageBase64) {
          images[img.fieldName] = img.imageBase64;
        }
      });
    }
    ...
    res.status(200).json({
      ...
      data: {
        page: {
          id: pageResult.data?.id || null,
          page_name: 'header',
          content: content,
          created_at: pageResult.data?.created_at || null,
          updated_at: pageResult.data?.updated_at || null
        },
        images: images
      }
    });
  } catch (error) {
    console.error('[CMS] Error fetching header data:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to fetch header data'
    });
  }
});
...
/**
 * POST - Save header data with content and images
 * POST /api/cms/header/save
 * Accepts: { content: {...}, images: {...} }
 */
router.post('/header/save', async (req, res) => {
  try {
    const { content, images } = req.body;
    
    console.log('[CMS] Saving header data...');
    console.log('[CMS] Header content keys:', content ? Object.keys(content) : 'No content');
    console.log('[CMS] Header image fields:', images ? Object.keys(images) : 'No images');
    
    // Save page content
    const pageResult = await saveCmsPage('header', content || {});
      ...
      const imagePromises = Object.entries(images).map(async ([fieldName, imageBase64]) => {
        try {
          if (!imageBase64 || typeof imageBase64 !== 'string') {
            imageResults[fieldName] = { success: false, error: 'Invalid image data' };
            return;
          }
          ...
          if (!imageData) {
            imageResults[fieldName] = { success: false, error: 'Invalid base64 data' };
            return;
          }
          ...
          imageResults[fieldName] = {
            success: result.success,
            imageId: result.imageId,
            error: result.success ? null : result.message
          };
        } catch (err) {
          console.error(`[CMS] Error saving header image ${fieldName}:`, err);
          imageResults[fieldName] = { success: false, error: err.message };
        }
      });
    ...
    console.log('[CMS] Header save complete:', {
      pageSuccess: pageResult.success,
      imageResults: Object.keys(imageResults).map(k => ({ field: k, success: imageResults[k].success }))
    });
    ...
    console.error('[CMS] Error saving header:', error);
    res.status(500).json({
      success: false,
      error: error.message || 'Failed to save header data'
    });
  }
});
