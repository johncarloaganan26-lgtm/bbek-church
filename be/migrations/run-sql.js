...
async function runSQL() {
  try {
    ...
    const [createResult] = await query(createTableSQL);
    console.log('Table created successfully:', createResult);

    // Add comment SQL
    const alterTableSQL = `ALTER TABLE \`tbl_audit_trail\` COMMENT = 'Audit trail table for tracking all user activities and system events';`;

    console.log('Adding table comment...');
    const [alterResult] = await query(alterTableSQL);
    console.log('Comment added successfully:', alterResult);
