-- =====================================================
-- AUDIT TRAIL TABLE v2 - Raw Data Only Storage
-- All fields store plain text, no Buffers, no IDs
-- =====================================================

CREATE TABLE
IF
  NOT EXISTS `tbl_audit_trail` (
    `audit_id` INT NOT NULL AUTO_INCREMENT COMMENT 'Primary key (kept for indexing)'
    , `user_email` VARCHAR(255) NULL COMMENT 'User email only - no user_id, name, or position'
    , `action_type` VARCHAR(50) NOT NULL COMMENT 'Action: CREATE, UPDATE, DELETE, LOGIN, LOGOUT, EXPORT, VIEW_LIST'
  ...
  ) ENGINE = InnoDB DEFAULT CHARSET = utf8mb4 COLLATE = utf8mb4_unicode_ci COMMENT = 'Audit trail v2 - All data stored as plain text, minimal fields';
  ...
  /*
     ...
     ('pastor@church.com', 'UPDATE', 'Water Baptism', 'Jane Smith - Baptism ID 456', 'Updated water baptism status from pending to approved via PUT /api/services/waterBaptism/updateStatus/456', '192.168.1.2', 'success'),
